...
def compute_heat_capacity(
    ...
    return summarize_blocks(cp_blocks)
...
def compute_bulk_modulus(
    ...
    for block in split_blocks(volumes, block_size):
        ...
        bulk_blocks.append((1.0 / kappa) / 1e9)
...
def get_thermo_props(
    ...
    for key, traj in data_dict.items():
        ...
        results[key] = {
            ...
            "enthalpy (kJ/mol)": compute_molar_enthalpy(
                traj,
                block_sizes.get("enthalpy"),
            ),
            "heat_capacity (kJ/mol/K)": compute_heat_capacity(
                traj,
                temperature,
                block_sizes.get("heat_capacity"),
            ),
            "compressibility (1/Pa)": compute_compressibility(
                traj,
                temperature,
                block_sizes.get("compressibility"),
            ),
            "bulk_modulus (GPa)": compute_bulk_modulus(
                traj,
                temperature,
                block_sizes.get("bulk_modulus"),
            ),
            "thermal_expansion (1/K)": compute_thermal_expansion(
                traj,
                temperature,
